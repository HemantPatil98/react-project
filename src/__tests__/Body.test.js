...
jest.mock("../utils/useOnlineStatus", () => () => true);

//mock useDebounce hook
jest.mock("../utils/useDebounce", () => (value) => value);

//mock useFetch API hook
...
it("Should search for burger", async () => {
  ...
  expect(resCardBeforeSearch.length).toBeGreaterThan(resCardAfterSearch.length);
});

it("Should filter restaurants with 4.0 and above rating", async () => {
  await act(async () => {
    render(
      <BrowserRouter>
        <Body />
  ...
  const ratingFilterBtn = screen.getByTestId("rating-filter");

  fireEvent.click(ratingFilterBtn);

  const resCardAfterFilter = screen.getAllByTestId("resCard");

  expect(resCardBeforeFilter.length).toBeGreaterThan(resCardAfterFilter.length);
});
